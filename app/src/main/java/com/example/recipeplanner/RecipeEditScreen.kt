...
@Composable
fun RecipeEditScreen(dish: Recipes?, onBack: () -> Unit, onSave: (Recipes, List<String>, List<String>) -> Unit) {
    var name by remember(dish?.id) { mutableStateOf(dish?.name ?: "") }
    var prepTimeMin by remember(dish?.id) { mutableStateOf(dish?.prepTimeMin?.toString() ?: "" ) }
    var cookTimeMin by remember(dish?.id) { mutableStateOf(dish?.cookTimeMin?.toString() ?: "") }
    var servings by remember(dish?.id) { mutableStateOf(dish?.servings?.toString() ?: "") }
    var meal by remember(dish?.id) { mutableStateOf(dish?.meal ?: "") }
    val image = dish?.image

    val ingredientsText = remember(dish?.id) { mutableStateListOf<String>().apply {
        addAll(dish?.ingredients ?: listOf(""))
        if (isEmpty()) add("")
    }}
    val directionsText = remember(dish?.id) { mutableStateListOf<String>().apply {
        addAll(dish?.directions ?: listOf(""))
        if (isEmpty()) add("")
    }}
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(text = "Edit Recipe", style = MaterialTheme.typography.headlineSmall) },
                navigationIcon = {
                        ...
                        when {
                            ...
                            !isInteger(servings) -> {
                                ...
                                val updatedRecipe = Recipes(
                                    ...
                                    meal = meal,
                                    image = image)
                                onSave(updatedRecipe, ingredientsText.toList(), directionsText.toList())
                            }
                        }
                    },
                    enabled = formFilled,
